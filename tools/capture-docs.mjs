...
const OUT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../docs/media/screenshots')
...
await send('Emulation.setDeviceMetricsOverride', { width: 1600, height: 970, deviceScaleFactor: 2, mobile: false })
...
function seedTasks (lang) {
  ...
  const rows = [
    ...
    T(zh ? '准备季度产品评审会 #重要' : 'Prepare quarterly review meeting #important', zh ? '数据看板截图 + 竞品对比页' : 'Dashboard shots + competitor page', 100001, { dayOffset: 0, estimate: 3, remind: 120, remindExtra: [-90, 0, 45], subs: zh ? [
  ...
  const mk = (t, dayOffset, complete, extra = {}) => {
    ...
    return {
      ...
      reminderExtra: (t.remindExtra || []).map(o => dayStart + 10 * 3600000 + o * 60000),
...
localStorage.setItem('onboardingToursSeen', JSON.stringify({ today: 1, pips: 1, editpanel: 1 }));
localStorage.removeItem('todayViewMode'); // 今日页视图态持久化：每次从列表视图开拍
'SEEDED'`
}
...
const PLAN = {
  'zh-CN': [
    ['01-today', '/todo-list/today', 'light', null],
    ['02-today-dark', '/todo-list/today', 'dark', null],
    ['03-calendar', '/todo-list/calendar', 'light', null],
    ['04-statistics', '/todo-list/statistics', 'light', null],
    ['05-matrix', '/todo-list/today', 'light', 'matrix'],
    ['06-todobox', '/todo-list/todo-box', 'light', null],
    ['08-calendar-dark', '/todo-list/calendar', 'dark', null],
    ['09-statistics-dark', '/todo-list/statistics', 'dark', null],
    ['11-tomato-record', '/todo-list/today', 'light', 'record'],
    ['12-editpanel-reminders', '/todo-list/today', 'light', 'edit'],
    ['13-deck', '/todo-list/today', 'light', 'deck']
  ]
}

for (const [lang, shots] of Object.entries(PLAN)) {
  await bootWith(lang)
  for (const [name, hash, theme, action] of shots) {
    await go(hash, theme)
    if (action === 'matrix' || action === 'deck') {
      // 今日页视图切换段：列表=0 矩阵=1 卡片=2
      const idx = action === 'matrix' ? 1 : 2
      await evaluate(`(function(){ var b=[...document.querySelectorAll('.pd-view-seg button')][${idx}]; if(b) b.click(); return 'ok' })()`)
      await sleep(1600)
    } else if (action === 'record') {
      await evaluate(`(function(){ window.appUI.$store.commit('ui/toggleTomatoFocusRecord', true); return 'ok' })()`)
      await sleep(1600)
    } else if (action === 'edit') {
      // 先关掉上一 scene 可能残留的记录弹窗，再开编辑栏
      await evaluate(`(function(){ var b=[...document.querySelectorAll('.pd-view-seg button')][0]; if(b) b.click(); window.appUI.$store.commit('ui/toggleTomatoFocusRecord', false); var t = window.appUI.$store.state.todo.todoList.find(x => x.taskId === 'seed_3'); window.appUI.$store.commit('ui/openEdit', t); return 'ok' })()`)
      await sleep(1800)
    }
    await shot(name)
    // 关闭弹层回到干净态（store 层显式关，合成 Esc 对弹层不可靠）
    await evaluate(`(function(){ try { window.appUI.$store.commit('ui/toggleTomatoFocusRecord', false) } catch(e){}; try { window.appUI.$store.commit('ui/closeEdit') } catch(e){}; return 'ok' })()`)
    await sleep(600)
  }
}
